...
function statsForJob(
  ...
  return {
    ...
    rejected: apps.filter(
      (a) => a.status === "rejected" || a.status === "regretted",
    ).length,
  };
}
...
export function JobAdvertsPage({
  ...
  const handleReopen = async (job: Job) => {
    if (!user) return;
    const newDate = await promptText({
      title: "New closing date (YYYY-MM-DD), or leave blank to keep none:",
      defaultValue: "",
    });
  ...
  const toggleSelectAll = () =>
    setSelected((prev) =>
      prev.length === filteredAdverts.length
        ? []
        : filteredAdverts.map((j) => j.id),
    );
  ...
  const handleArchiveSelected = async (archived: boolean) => {
    ...
    try {
      ...
      toast.success(
        `${selected.length} advert(s) ${archived ? "archived" : "restored to active"}.`,
      );
      setSelected([]);
      regenerateJobsFeed().catch(() => {});
      load();
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          ...
          <p className="text-gray-500">
            Manage your published job postings and their visibility.
          </p>
        </div>
              ...
              <tr>
                <th className="px-6 py-4 text-left w-12">
                  <input
                    type="checkbox"
                    checked={
                      selected.length === filteredAdverts.length &&
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-gray-900">
                            {job.title}
                            {job.isConfidential && (
                              <span className="ml-2 text-[10px] uppercase text-purple-600 font-bold">
                                Confidential
                              </span>
                            )}
                            {job.advertType === "internal" && (
                              <span className="ml-2 text-[10px] uppercase text-blue-600 font-bold">
                                Internal
                              </span>
                            )}
                          </span>
                          <span className="text-xs text-gray-500">
                            {job.referenceNumber} • {job.location || "—"}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          {(job.status === "Active" ||
                            job.status === "Re-advertised") &&
                            job.advertType === "external" && (
                              <div className="relative">
                                <button
                                  onClick={() =>
                                    setShareJobId(
                                      shareJobId === job.id ? null : job.id,
                                    )
                                  }
                ...
                <tr>
                  ...
                  >
                    ...
                    <p className="text-lg font-medium text-gray-900">
                      No job adverts found
                    </p>
                    <p className="text-sm text-gray-500 mt-1">
                      Create your first advert or adjust the filters.
                    </p>
                  </td>
                </tr>
              )}
